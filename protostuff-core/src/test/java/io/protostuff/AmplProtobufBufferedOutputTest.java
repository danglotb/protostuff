 ...
 * @created Oct 7, 2010
 */
public class AmplProtobufBufferedOutputTest extends SerDeserTest {
    @Override
    protected <T> void mergeDelimitedFrom(java.io.InputStream in, T message, Schema<T> schema) throws java.io.IOException {
        ProtobufIOUtil.mergeDelimitedFrom(in, message, schema);
    }

    @Override
    protected <T> void writeDelimitedTo(java.io.OutputStream out, T message, Schema<T> schema) throws java.io.IOException {
        ProtobufIOUtil.writeDelimitedTo(out, message, schema, AbstractTest.buf());
    }
    ...
    protected <T> byte[] toByteArray(T message, Schema<T> schema) {
        return ProtobufIOUtil.toByteArray(message, schema, AbstractTest.buf());
    }
    ...
    public void testNestedOverflow() {
        ...
        bar.setSomeString(SerDeserTest.repeatChar('a', repeat));
        byte[] data = ProtobufIOUtil.toByteArray(bar, bar.cachedSchema(), AbstractTest.buf(bufSize));
        ...
        data = ProtobufIOUtil.toByteArray(bar, bar.cachedSchema(), AbstractTest.buf(bufSize));
    ...
    public void testNestedLarge() {
        ...
        // =====
        // 131
        bar.setSomeBaz(new Baz(10, SerDeserTest.repeatChar('b', 125), 15L));
        // size will be:
        // 1 (tag)
